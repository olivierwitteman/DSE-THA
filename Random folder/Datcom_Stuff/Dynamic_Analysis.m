...
C_la= 6.78; %Lift curve slope, taken from data sheet provided by Martin dCl/dalpha[1/rad]  ????
S= 9.52; %Wing surface area [m^2] ???? STAN1
...
rho = 0.966632; %Density at 2400m altitude [kg/m^3]
...
C_zq = -2.*C_la/1.66*Vh_V^2*(Sh*lh)/(S*c)  ; %-5.6629; % Derivative of coefficient of Z-force in Z-direction with respect to pitching velocity
C_mq =   -1.15*C_la/1.66*Vh_V^2*(Sh*lh^2)/(S*c^2); %-8.7941; % Derivative of the pitching moment coefficient with respect to pitching velocity
% C_zq = C_mq*1.03/(1.1*lh)
C_zar = -C_la/1.66*de_da*Vh_V^2*(Sh*lh)/(S*c) ; %-0.0035; % Derivative of coefficient of Z-force in Z-direction with respect to the change in the angle of attack
C_mar = -C_la/1.66*de_da*Vh_V^2*(Sh*lh^2)/(S*c^2) ; %0.1780; % Derivative of the pitching moment coefficient with respect to the change in the angle of attack
...
sizeN = 20;
plot(X1,YMatrix1,'DisplayName','Symmetric Short Period','MarkerFaceColor',[0.0 0.0 0.0],'MarkerEdgeColor',[0.0 0.0 0.0],...
    'MarkerSize',sizeN,...
    'Marker','o',...
    'LineStyle','none',...
    'Color',[1 0 0]); hold on;
plot(X2,YMatrix2,'DisplayName','Symmetric Phugoid','MarkerFaceColor',[1 0 0],'MarkerSize',sizeN,'Marker','o',...
    'LineStyle','none',...
    'Color',[1 0 0]); hold on;
plot(X3,YMatrix3,'DisplayName','Asymmetric Aperiodic Roll','MarkerFaceColor',"Magenta",'MarkerEdgeColor',"Magenta",...
    'MarkerSize',sizeN,...
    'Marker','o',...
    'LineStyle','none',...
    'Color',[1 0 0]); hold on;
