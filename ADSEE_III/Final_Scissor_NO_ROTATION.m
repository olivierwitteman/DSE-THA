...
x_datum_h =6.8 ;                  % <------ INPUT     measured from planform for given geometry (from nose to horizontal tail) [m]
%--------------------------------------------------------------------------
...
CL_A_h = 1.03+0.2;% lift coefficient of wing+fuselage (without tail, landing configuration)
% lecture 4 slide 37

CL_h = -0.35*Ah^(1/3); % SLIDE 16

CL_0 = 0.8563-0.5; % 
Cm0 = -0.216+0.1; %  (for main wing) [-]
...
mu_3=0.55-0.5*0;% accounts for the effect of sweep angle SHIFT TO THE LEFT THE GRAPH
cf_c= 1.33-0.15-0.1; %extended chord due to flaps and normal chord length
S_wf_S=1.33-0.15-0.10; %ratio between flapped wing area and reference wing area 
...
for i  = x_lemac
    ...
    seat_pilot=2.2-1.0 ;                    %c.g. Position Pilot   <----- INPUT 
    seat_row1=2.7-1.0  ;                    %c.g. Position Row 1   <----- INPUT 
    seat_row2=4.0-1.0  ;                    %c.g. Position Row 2   <----- INPUT 
    location_cargo = 4.6;             %c.g. Position Baggage <----- INPUT
    location_fuel = 2.63+0.5;                %c.g. Position Fuel    <----- INPUT

    location_fuel = i + 0.4*MAC; % changing with shifting mac
...
legend({"FORWARD CG", "AFT CG", "Stability", "Neutral Stability", "Controlability"}, "FontSize", 30)
%set(gca,'FontSize',30);
