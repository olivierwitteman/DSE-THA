...
x_datum_h =7.5;                  % <------ INPUT     measured from planform for given geometry (from nose to horizontal tail) [m]
%--------------------------------------------------------------------------
...
mu_1=0.17+0.5*0; % accounts for the effect of the camber increase associated with the deflection angle of the flaps
mu_2=0.5-0.2;%represent the 2D effect lift contribution generated by the high lift devices
mu_3=0.55-0.55;% accounts for the effect of sweep angle
cf_c= 1.33-0.2; %extended chord due to flaps and normal chord length
S_wf_S=1.33-0.33*0; %ratio between flapped wing area and reference wing area 
...
for i  = x_lemac
    ...
    cg_OEW = i + 0.3*MAC;                  % BIGGEST INPUT
...
%     location_fuel = 3.1;                %c.g. Position Fuel    <----- INPUT
    location_fuel = i + 0.4*MAC % MAYBE
