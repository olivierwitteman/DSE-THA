...
Sh = 1.48; %Area of the horizontal tail wing [m]
...
zh = 0.2;%vertical distance between wing and tail root chord taken from current geometry [m] (ASSUMED)
...
%% Controllability
CLAh = 1.6;% lift coefficient of wing+fuselage (without tail, landing configuration)
% lecture 4 slide 37
...
CLmax= 2.2; %aircraft lift at landing
dClmax = 0.37;
...
%OEW = double(vars.OEW);
OEW=1768;
cg_OEW_range = [2 : 0.01 : 5]
x_lemac = 2.9;
% x_lemac = [2, 3, 4]
cg_mat = zeros(length(cg_OEW_range),2);
xlemacfus = zeros(length(cg_OEW_range), 1)
counter = 1
...
%     location_feul = x_Fuel;
    ...
    cg_max = (cg_max - x_lemac)/MAC;
    cg_min = (cg_min - x_lemac)/MAC;
    cg_mat(counter, 1) = cg_min;
    cg_mat(counter, 2) = cg_max;
    
    xlemacfus(counter,1) = x_lemac/lf
...
%figure
yyaxis left
plot([cg_mat(:,1), cg_mat(:,2)], xlemacfus/lf, "LineWidth", thick)
