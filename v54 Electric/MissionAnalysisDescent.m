function [AFlipped] = MissionAnalysisDescent(a,p,f,s,c,con,aircraft,A,h_target,M_target,dt)
...
while (A.h(k) < h_target || A.v(k) < M_target*f.a(h_target)) 
    ...
    if h >= 0 && h <= h_target
        xi = interp1([0 h_target],A.xi,h_target-h,'linear');
        phi = interp1([0 h_target],A.phi,h_target-h,'linear');
        Phi = interp1([0 h_target],A.Phi,h_target-h,'linear');
    else
    ...
    if SEP > 0
        disp(['Warning: Positive SEP found during descent (iteration '...
                num2str(k) '). Lower thrust settings.'])
    end
    
    % Interpolate share between climb- and accelerate SEP
    x = interp1(h_array,x_array,(h_target-h),'linear');
    ...
    if k > 5*s.itermax
        if strcmp(con,'Dde')
            error('Could not reach diversion altitude/Mach number during descent')
        else
            error('Could not reach cruise altitude/Mach number during descent')
        end
    end
end
