...
n_max = 4.4*1.5; %maximum loading factor (CS23.333)
n_min = -0.4*n_max; % minimum loading factor (CS23.333)

%% V-n Diagram - axis and square curve

 V_S = sqrt((2*MTOW)/(1.60*1.225*S))*1.5; % stall speed (EAS) [m/s] 1.5 was Cl_max
% V_S = 33.0;
% V_A = sqrt((n_max*2*MTOW)./(rho*Cl_max*S));  %design maneuver speed at max loading factor (EAS) [m/s] 
V_A = V_S*sqrt(n_max)*1.5;
V_A = 0:0.1:V_A; % vector for the first curve up until V_A
n = (V_A.^(2)*((1.225+0.06*0)*1)*1.60*S)./(2*MTOW);   % vector for load factors corresponging to V_A_eas it was 0.9664
...
%V_C = sqrt((2*MTOW)/(1.225*S*Cl)); %cruise speed at max loading factor (EAS)[m/s]
V_C = 85.37*1.5;

V_D = V_C*1.5*1.5; %design dive speed (CS23.333), (EAS) [m/s]
...
V_S_neg = sqrt((2*abs(n_min)*MTOW)./(rho*Cl_max*S))*1.5; %negative stall speed (TAS) [m/s]
...
txt1 = 'U_{b} = 20.1 [m/s]'; %for dive speed
text(x1,y1,txt1,'FontSize',18) %for dive speed
...
txt1 = 'U_{c} = 15.2 [m/s]'; %for dive speed
text(x1,y1,txt1,'FontSize',18) %for dive speed
...
txt1 = 'U_{d} = 7.6 [m/s]'; % for dive speed
text(x1,y1,txt1,'FontSize',18) % for dive speed
