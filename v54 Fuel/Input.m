...
% Cruise
p.cr.etap1 = 0.65;                              % Primary propulsors' propulsive efficiency in cruise (of ISOLATED propulsors) [-]
...
p.TO.etap2 = 0.7;                              % Secondary propulsors' propulsive efficiency in TO conditions (of ISOLATED propulsors) [-]
...
m.cr.phi = 0.05;                                 % Cruise supplied power ratio [-]
m.cr.Phi = 0;                                 % Cruise shaft power ratio [-]

% Landing
m.L.h = 0;                                      % Landing altitude [m]
m.L.f = 0.92;                                    % Landing weight fraction W/MTOW [-]
...
m.L.t = 0.1;                                      % Landing throttle setting P/P_max [-] (see note at end)
...
m.ct.f = 0.95;                                    % Start-of-climb weight fraction W/MTOW [-]
...
m.ct.phi = 0;                                % Top-of-climb supplied power ratio [-]
...
MA_in.M_div = 0.2797;                              % Diversion cruise Mach number [-]
...
% p.dy = 0;                                    % Spacing between adjacent DP propulsors, as fraction of propulsor diameter [-]
% p.N1 = 1;                                       % Number of chains in primary powertrain [-]
% p.N2 = 0;                                       % Number of chains in secondary powertrain[-]
% p.DP = 0;                                       % Which PS has an effect on wing performance? (1 = primary, 2 = secondary, 0 = none)
...
% p.eta_GB = 0.95;                                % Transmission efficiency of gearboxes
% p.eta_GT = 0.35;                                % Conversion (thermal) efficiency of gas turbine
% p.SE.bat = 0.95e6;                               % Battery specific energy [J/kg] 
% p.SE.f = 42.8e6;                                % Fuel specific energy [J/kg]
% p.SP.EM = 7.5e3;                                % Electrical machine specific power [W/kg]
...
% p.cr.etap2 = 0.0;                              % Secondary propulsors' propulsive efficiency in cruise (of ISOLATED propulsors) [-]
% p.cr.Gamma = 0;                                 % Thrust vectoring in cruise [deg]
% 
% % Landing
% p.L.etap1 = 0.8;                                % Primary propulsors' propulsive efficiency in landing conditions (of ISOLATED propulsors) [-]
% p.L.etap2 = 0.0;                                % Secondary propulsors' propulsive efficiency in landing conditions (of ISOLATED propulsors) [-]
% p.L.Gamma = 0;                                  % Thrust vectoring in landing configuration [deg]
% 
% % Take off
...
% m.TO.phi = 0;                                 % TO supplied power ratio [-]
% m.TO.Phi = 0;                                   % TO shaft power ratio [-]
% 
% % OEI Balked landing
% m.bL.G = 0.027;                                 % OEI balked landing climb gradient [-] (CS25.121d)
...
% m.bL.vMargin = 1.4;                             % Stall margin in balked-landing conditions
% m.bL.t = 1;                                     % Balked landing throttle setting P/P_max [-] (see note at end)
% m.bL.phi = 0;                                 % Balked landing supplied power ratio [-]
% m.bL.Phi = 0;                                   % Balked landing shaft power ratio [-]
% 
% % OEI ceiling
% m.cI.h = 1200;                                  % OEI ceiling [m]
...
% m.cI.phi = 0;                                 % OEI-ceiling landing supplied power ratio [-]
% m.cI.Phi = 0;                                   % OEI-ceiling landing shaft power ratio [-]
% 
% % Start-of-climb
% m.cl.h = 0;                                     % Altitude for start-of-climb constraint [m]
...
% m.cl.phi = 0;                                % Start-of-climb supplied power ratio [-]
% m.cl.Phi = 0;                                   % Start-of-climb shaft power ratio [-]
% 
% % Top-of-climb
% m.ct.h = 2400;                                 % Altitude for top-of-climb constraint [m]
...
% 
% % Diversion cruise (level flight is specified, so one DOF must be kept free)
% MA_in.Dcr.xi = [0.8 0.8];
% MA_in.Dcr.phi = [0 0]; 
% MA_in.Dcr.Phi = [0 0]; 
% 
% % Diversion descent (all parameters must be specified; SEP is computed)
% MA_in.Dde.xi = [0.4 0.4]; 
% MA_in.Dde.phi = [0 0]; 
% MA_in.Dde.Phi = [0 0]; 
% 
% 
