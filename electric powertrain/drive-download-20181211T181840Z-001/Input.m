...
p.config = 'e-2';                      % Powertrain architecture ('conventional', 'turboelectric', 'serial',
                                                %   'parallel', 'PTE', 'SPPH', 'e-1', 'e-2', or 'dual-e')
p.b_dp = 0.6;                                   % Fraction of wing span occupied by DP system [-]
p.dy = 0.07;                                    % Spacing between adjacent DP propulsors, as fraction of propulsor diameter [-]
p.N1 = 2;                                       % Number of chains in primary powertrain [-]
p.N2 = 10;                                      % Number of chains in secondary powertrain[-]
...
p.SE.bat = 1.8e6;%610e3;                        % Battery specific energy [J/kg] 
p.SE.f = 42.8e6;                                % Fuel specific energy [J/kg]
p.SP.EM = 7.5e3;                                % Electrical machine specific power [W/kg]
p.SP.bat = 1e3;                                 % Battery pack specific power [W/kg]
...
% Take off
p.TO.etap1 = 0.60;                              % Primary propulsors' propulsive efficiency in TO conditions (of ISOLATED propulsors) [-]
...
% OEI Balked landing
p.bL.etap1 = 0.6;                               % Primary propulsors' propulsive efficiency in balked landing conditions (of ISOLATED propulsors) [-]
...
% Cruise
m.cr.h = 2500;                                 % Cruise altitude [m]
m.cr.M = 0.279;                                  % Cruise Mach number [-]
m.cr.f = 1.00;                                  % Cruise weight fraction W/MTOW [-]
m.cr.t = 0.9;                                     % Cruise throttle setting P/P_max [-] (see note at end)
m.cr.phi = 1;                                   % Cruise supplied power ratio [-]
m.cr.Phi = 0.5;                                 % Cruise shaft power ratio [-]

% Landing
m.L.h = 0;                                      % Landing altitude [m]
m.L.f = 1;                                    % Landing weight fraction W/MTOW [-]
m.L.vs = 32;                                  % Stall speed requirement in landing conditions [m/s]
...
m.bL.phi = 1;                                 % Balked landing supplied power ratio [-]
m.bL.Phi = 0.5;                                   % Balked landing shaft power ratio [-]

% OEI ceiling
m.cI.h = 2200;                                  % OEI ceiling [m]
m.cI.f = 1;                                  % OEI-ceiling weight fraction W/MTOW [-]
...
m.cI.phi = 1;                                 % OEI-ceiling landing supplied power ratio [-]
...
m.cl.v = 50;                                   % Velocity at start-of-climb (shoud be equal to V2 obtained from TO constraint) [m/s]
...
m.cl.phi = 1;                                % Start-of-climb supplied power ratio [-]
m.cl.Phi = 0.5;                                   % Start-of-climb shaft power ratio [-]

% Top-of-climb
m.ct.h = 2500;                                 % Altitude for top-of-climb constraint [m]
m.ct.f = 1;                                     % Start-of-climb weight fraction W/MTOW [-]
m.ct.M = 0.279;                                  % Mach at top-of-climb (shoud be equal to cruise Mach) [-]
m.ct.G = 0.008;                                  % Top-of-climb climb gradient [-] (based on MA observances)
m.ct.dVdt = 0.05;                               % Top-of-climb acceleration [m/s2]
m.ct.t = 1.0;                                   % Top-of-climb throttle setting P/P_max [-] (see note at end)
m.ct.phi = 1;                                % Top-of-climb supplied power ratio [-]
...
MA_in.R = 463e3;                              % Range [m]
MA_in.R_div = 187e3;                          % Diversion range [m]
MA_in.h_div = 1500;                             % Diversion altitude [m]
MA_in.M_div = 0.15;                              % Diversion cruise Mach number [-]
                                                % Nominal mission M and h are specified in the
                                                % "m" structure (cruise constraint)
% Initial guesses for convergence loop
MA_in.OEM = 600;                              % Operative empty mass incl. powertrain and wing, excl. bat [kg]
...
MA_in.cl.xi = [0.95 0.95];
MA_in.cl.phi = [1 1];
MA_in.cl.Phi = [0.5 0.5];
