...
'The aileron inner chord equals (meters):' 
(c_r - b1(i)*(tan(degtorad(lambda))+tan(degtorad(theta))))*chordratio_ail_total(j))
           , disp('The aileron outer chord equals (meters):'), disp() 
(c_r - b2(i)*(tan(degtorad(lambda))+tan(degtorad(theta))))*chordratio_ail_total(j)