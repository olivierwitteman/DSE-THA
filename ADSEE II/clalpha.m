...
%values of figures. As this sizing is highly statistical and perliminary.
%General inputs
A=25;
sweepc2 =1; 
S=35;
...
alpha0L=-9.; %Given by the airfoil
CLdes=0.65; %Given
taper=0.6; %Given
Sharpfactor=0.02; %from the airfoil.
clmax=1.2; %this is the clmax for the airfoil (slide 17)
%%statistical
%general
...
HLD_Delta_CLmax=0.9*Delta_Clmax_HLD*Swf/S*cos(sweep_hinge); %The difference in CLmax
%due to the high lift devices.
HLD_Delta_alpha0L=delta_alpha0L_airfoil*Swf/S*cos(sweep_hinge); %Same but than 
%0L angle of attack.
%%
%CLalpha for flapped configuration
%if HLD increase wing surface
S_ac_S= 1+ Swf/S*(c_ac_c-1) ;% S'/S
