...
p.config = 'serial';                      % Powertrain architecture ('conventional', 'turboelectric', 'serial',
...
p.N2 = 8;                                       % Number of chains in secondary powertrain[-]
...
% Cruise
m.cr.h = 11280;                                 % Cruise altitude [m]
m.cr.M = 0.78;                                  % Cruise Mach number [-]
...
m.cr.Phi = 1;                                 % Cruise shaft power ratio [-]

% Landing
m.L.h = 0;                                      % Landing altitude [m]
m.L.f = 0.9;                                    % Landing weight fraction W/MTOW [-]
m.L.vs = 57.7;                                  % Stall speed requirement in landing conditions [m/s]
...
m.L.Phi = 1;                                    % Landing shaft power ratio [-]
...
m.TO.Phi = 1;                                   % TO shaft power ratio [-]
...
m.cl.Phi = 1;                                   % Start-of-climb shaft power ratio [-]

% Top-of-climb
m.ct.h = 11280;                                 % Altitude for top-of-climb constraint [m]
m.ct.f = 1;                                     % Start-of-climb weight fraction W/MTOW [-]
m.ct.M = 0.78;                                  % Mach at top-of-climb (shoud be equal to cruise Mach) [-]
...
MA_in.PL = 20000;                               % Payload [kg]
MA_in.R = 2000e3;                               % Range [m]
MA_in.R_div = 463e3;                            % Diversion range [m]
MA_in.h_div = 3000;                             % Diversion altitude [m]
MA_in.M_div = 0.5;                              % Diversion cruise Mach number [-]
                                                % Nominal mission M and h are specified in the
                                                % "m" structure (cruise constraint)
% Initial guesses for convergence loop
MA_in.OEM = 20000;                              % Operative empty mass incl. powertrain and wing, excl. bat [kg]
MA_in.FF_tot0 = 0.2;                            % Fuel fraction (excl. batteries, incl. diversion) of aircraft [-]
